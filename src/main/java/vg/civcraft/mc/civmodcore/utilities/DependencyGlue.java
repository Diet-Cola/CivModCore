...
public abstract class DependencyGlue implements Listener {
	...
	protected DependencyGlue(@Nonnull final Plugin plugin, @Nonnull final String dependencyName) {
		...
		Preconditions.checkArgument(this.pluginManager.isPluginEnabled(plugin));
		Preconditions.checkArgument(!StringUtils.isBlank(dependencyName));
		this.plugin = plugin;
		this.logger = plugin.getLogger();
		this.dependencyName = dependencyName;
		this.dependencyInstance = this.pluginManager.getPlugin(dependencyName);
	}
